...
static PyTypeObject StructObject_Type = {
...
static PyObject * mxToPy(mxArray *mx)
{
  ...
  size_t nd = mxGetNumberOfDimensions(mx);
  size_t *dims = (size_t*)mxGetDimensions(mx);
  ...
  case mxUINT64_CLASS:
    ...
    else if(nd == 2 && dims[0] == 1) {
      nd = 1;
      dims = &dims[1];
    }
    else if(nd == 2 && dims[1] == 1) {
      nd = 1;
      dims = &dims[0];
    }
    return PyArray_New(&PyArray_Type,nd,(npy_intp*)dims,type_num,NULL,data,itemsize,NPY_F_CONTIGUOUS,NULL);
  case mxCHAR_CLASS:
    return PyString_FromString(mxArrayToString(mx));
  case mxSTRUCT_CLASS:
    {
      PyObject *obj = PyObject_CallObject((PyObject*)&StructObject_Type, NULL);
...
static mxArray * pyToMx(PyObject *py)
{
  ...
  size_t *dims = (size_t*)PyArray_DIMS(arr);
  ...
  case NPY_OBJECT:
    if(nd == 0) {
      mxArray *mx = mxCreateStructArray(1,zerodim,0,NULL);
      PyObject *dict = PyObject_GetAttrString(py,"__dict__");
      PyObject *key, *value;
      Py_ssize_t pos = 0;
      while(PyDict_Next(dict,&pos,&key,&value)) {
	char *fieldname = PyString_AsString(key);
	if(fieldname[0] != '_') {
	  mxAddField(mx,fieldname);
	  mxSetField(mx,0,fieldname,pyToMx(value));
	}
      }
      return mx;
    }
